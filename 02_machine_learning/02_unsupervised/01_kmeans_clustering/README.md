...
**MLCourse · Machine Learning · unsupervised · 0_kmeans_clustering**
...
**Contents**
. [The idea in words](#-the-idea-in-words)
...
```
. PLACE     drop k centroids into the data (randomly, or smartly - see §4)
...
Plot WCSS against k = , 2, 3, … Each added cluster buys less improvement;
the curve bends like an arm. Pick the k at the **elbow** - the point where
adding another cluster stops paying rent.

```python
wcss = [KMeans(n_clusters=k, n_init=0, random_state=42).fit(X).inertia_
        for k in range(, )]
```
...
(e.g., salary 0,000-00,000) will drown one measured in small units
(e.g., age 8-70). Distance becomes "whoever has the biggest unit wins."

```python
from sklearn.preprocessing import StandardScaler
X_scaled = StandardScaler().fit_transform(X)   # mean 0, std  per feature
```

>  **Unscaled K-Means is the # beginner bug.** If one feature dominates
...
| `n_init` | 0 | independent restarts; best inertia wins | ≥ 0 (older defaults were riskier) |
...
| `tol` | e-4 | convergence threshold on centroid shift | rarely touched |
...
. **Local optima** -> different seeds, different clusters. Fix: `n_init≥0`
...
├── 0_theory_and_mathematics.nb.py      <- Lloyd's loop BY HAND + elbow (mall)
├── 02_model_development_workflow.nb.py  <- full scaled workflow + profiles (mall)
├── 0_easy_mall_segments.nb.py          <- PROJECT: classic 5-segment story
├── 02_medium_iris_blindfolded.nb.py     <- PROJECT: rediscover iris w/o labels
└── 03_hard_wine_red_chemistry.nb.py     <- PROJECT: do chemistry clusters track quality?
```
...
wcss, sils = [], []
for k in range(2, ):                                     # 2. sweep k
    km = KMeans(n_clusters=k, n_init=0, random_state=42).fit(Xs)
    wcss.append(km.inertia_)                               #    elbow fuel
    sils.append(silhouette_score(Xs, km.labels_))          #    silhouette fuel

km = KMeans(n_clusters=5, n_init=0, random_state=42).fit(Xs)   # 3. final fit
...
| reproducible run | `random_state=42, n_init=0` |

*Next: watch Lloyd's loop crawl in `0_theory_and_mathematics.nb.py` - by hand.*
