...
> ⚠️ **Inertia always falls as k grows** - with k = n you get WCSS = 0 (every
...
> ⚠️ **Unscaled K-Means is the #1 beginner bug.** If one feature dominates
...
1. **Local optima** → different seeds, different clusters. Fix: `n_init≥10`
...
├── 01_theory_and_mathematics.nb.py      <- Lloyd's loop BY HAND + elbow (mall)
├── 02_model_development_workflow.nb.py  <- full scaled workflow + profiles (mall)
├── 01_easy_mall_segments.nb.py          <- PROJECT: classic 5-segment story
...
*Next: watch Lloyd's loop crawl in `01_theory_and_mathematics.nb.py` - by hand.*
