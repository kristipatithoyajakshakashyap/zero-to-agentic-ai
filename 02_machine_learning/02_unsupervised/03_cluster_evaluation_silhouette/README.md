...
**Contents**
1. [The coefficient, point by point](#1-the-coefficient-point-by-point)
...
## 1. The coefficient, point by point
...
home". Dividing by `max(a,b)` squashes it into [-1, 1] regardless of scale.

| Situation | a | b | s |
|---|---|---|---|
| deep inside a tight island | small | large | -> +1 |
...
| Mean s | Verdict |
|---|---|
| 0.70 - 1.00 | strong structure, well separated |
...
```python
scores = {k: silhouette_score(Xs, km_k.labels_) for k in range(2, 11)}
...
- `silhouette_score(..., sample_size=10_000)` subsamples big data.
 ...
 **Avoid / distrust when…**
- n is huge without sampling - cost is **O(n²)** distances (150 points fine;
  1M rows painful)
...
1. **Trusting the mean alone** - a 0.55 can hide half great / half terrible.
...
├── 01_theory_and_mathematics.ipynb      <- compute s(i) BY HAND for 5 iris flowers
├── 02_model_development_workflow.ipynb  <- elbow+silhouette combo, diagrams k=5/k=2
└── projects/
    ├── 01_easy_mall_ksweep.ipynb        <- PROJECT: sweep k, diagram k=3/5/7
    ├── 02_medium_iris_diagrams.ipynb    <- PROJECT: honest low scores explained
    └── 03_hard_penguins_compare.ipynb   <- PROJECT: k AND algorithm selection
```
...
best_k = max(rows, key=lambda t: t[1])[0]

samples = silhouette_samples(Xs, final_labels)          # per-point diagnosis
worst = df.loc[samples.argsort()[:10]]                  # 10 least-at-home points
...
| fast on big data | `silhouette_score(Xs, labels, sample_size=10_000, random_state=42)` |
