...
$$P(\text{class} \mid x) \propto P(\text{class}) \times P(x_\mid\text{class}) \times P(x_2\mid\text{class}) \cdots$$
...
| `alpha` (Multinomial/Bernoulli) | Laplace smoothing for unseen words; .0 default |

No scaling needed; GaussianNB is scale-invariant.

## Pitfalls
- zero-frequency trap -> keep `alpha ≥ `
- probabilities are over-confident but rankings remain useful
- don't feed one-hot dummies AND their source column together

## Contents
- `0_theory_and_mathematics.ipynb` - Bayes computed BY HAND on Titanic counts
...
Pipeline([("bow", CountVectorizer()), ("nb", MultinomialNB(alpha=.0))])
```
