...
| `learning_rate` | step shrinkage ν | 0.05-0. |
...
| `early_stopping_rounds` | patience on eval metric | 50-00 |

 **Version churn alert**: in xgboost ≥ .6 (and all of 3.x),
...
- `get_booster().get_score(importance_type="gain")` may return keys like
  `f0, f, …` when you fit on NumPy arrays - map them back to column names
...
- extrapolation: trees can't predict outside the training range (and capped
  targets, like California's 5.0000 ceiling, cap predictions forever)

## Contents
- `0_theory_and_mathematics.nb.py` - one split's gain priced BY HAND, λ sweep, NaN-routing autopsy
- `02_model_development_workflow.nb.py` - early stopping + gain audit on California housing
...
model = XGBRegressor(
    ...
    subsample=0.85, colsample_bytree=0.85, reg_lambda=.0,
