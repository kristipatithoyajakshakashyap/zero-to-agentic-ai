...
$$\hat{y}(q)=\frac{}{k}\sum_{i \in \text{kNN}} y_i \quad\text{or}\quad \hat{y}(q)=\frac{\sum_i y_i/d_i}{\sum_i /d_i}$$

Classification votes; regression averages. Everything else is identical.

**Why scaling is MANDATORY:** a feature in people-per-household (0-0) is
...
| k | behavior | risk |
|---|---|---|
| too small (-3) | prediction jumps around, chases noise | overfit (high variance) |
| balanced (0-50) | smooth local averages | sweet spot |
...
## Contents
- `0_theory_and_mathematics.nb.py` - distances & neighbor-averaging by hand on real penguins
- `02_model_development_workflow.nb.py` - end-to-end sklearn workflow on California housing
...
pred = pipe.predict(X_test)     # = weighted average of the 5 nearest rows
```
