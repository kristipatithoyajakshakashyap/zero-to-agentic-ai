...
- Why the weak learner stays tiny (depth- / depth-2 trees) and why that is a feature
...
. Give every row equal weight `w_i = /n`.
2. Fit a shallow tree `h_m` respecting the current weights.
3. Score every row's error, then **divide by the worst error** so all errors land in `[0, ]`
   (this scaling is what lets regression borrow boosting machinery built for 0/ mistakes).
4. Convert errors to a `loss` in `[0, ]` - see table below - and average them with the
   current weights to get the round's quality score `err_m` (smaller is better).
5. Turn that into an **estimator weight** `alpha_m = ln(( - err_m)/err_m)`:
...
6. **Re-weight the rows:** well-predicted rows lose weight, poorly-predicted rows keep
   ...
   This is THE move: round `m+` cannot just repeat round `m`.
...
## Why weak depth- / depth-2 trees?

- A depth- tree (a "stump") asks ONE yes/no question about one feature - barely better
...
After scaling errors to `[0, ]` via division by the maximum error `D`:
...
| `"exponential"` | ` - exp(-\|err_i\| / D)` | compresses; most forgiving -> rounds survive longest |
...
- Teaching/interpreting: the weight dynamics are very visual (see notebook 0).
...
- Start `max_depth=` or `2` on the base tree; deeper stumps defeat the purpose.
- Keep `learning_rate <= .0`; it shrinks every `alpha_m` (gentler re-weighting).
...
|---|---|---|
| `estimator` | `DecisionTreeRegressor(max_depth=3)` | the weak learner; pass `DecisionTreeRegressor(max_depth=)` for true stumps |
| `n_estimators` | `50` | max boosting rounds (may end EARLY if a round hits `err >= 0.5`) |
| `learning_rate` | `.0` | shrinks each tree's voice `alpha_m`; smaller = safer, needs more rounds |
...
-  **Silent early stopping:** if `err_m >= 0.5`, sklearn discards the tree and stops;
  your `n_estimators=300` may really be 2. Check `len(estimators_)`.
...
| File | Focus |
|---|---|
| `0_theory_and_mathematics.nb.py` | hand-run 3 weighted rounds on diabetes (s5), weight-shift visuals, verified vs sklearn internals |
| `02_model_development_workflow.nb.py` | full workflow on diabetes: rounds curve, lr × loss grid, importances |
| `projects/0_easy_diabetes_adaboost.nb.py`  | EDA -> tuned AdaBoost vs single stump table |
| `projects/02_medium_mpg_adaboost.nb.py`  | messy mpg: median-fill, one-hot, rounds curve, residual diagnostics |
| `projects/03_hard_california_staged.nb.py`  | 6k-row California: R²-vs-rounds curve, lr race, extrapolation limits |
...
ada = AdaBoostRegressor(
    estimator=DecisionTreeRegressor(max_depth=),  # the weak learner (a stump)
...
GridSearchCV(AdaBoostRegressor(DecisionTreeRegressor(max_depth=), random_state=42),
             {"n_estimators": [00, 300], "learning_rate": [0.05, 0.3, .0],
