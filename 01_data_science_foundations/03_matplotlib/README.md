...
10. [Colormaps](#10-colormaps)
11. [Styles & rcParams](#11-styles--rcparams)
12. [Pandas integration](#12-pandas-integration)
13. [Saving figures](#13-saving-figures)
...
| good for | quick exploration | scripts, dashboards, anything >1 plot |
...
```python
ax.set(xlim=(0, 10), ylim=(-1, 1), xlabel="t", ylabel="v", title="signal")
```
...
```python
ax.set_title("...", fontsize=14, fontweight="bold", loc="left")
fig.suptitle("board title")                 # whole-Figure heading
ax.legend(loc="upper left", ncols=2, frameon=False)
ax.axhline(y, color="red", ls="--"); ax.axvspan(x0, x1, alpha=.15)   # refs/bands
...
`b, logA = np.polyfit(np.log10(x), np.log10(y), 1)`.
...
```python
im = ax.imshow(matrix, cmap="viridis", extent=[x0,x1,y0,y1], origin="lower")
...
ax.fill_between(x, mean-2*sem, mean+2*sem, alpha=.18, label="±2 SE")
```
...
| categories | qualitative | tab10, Set2, colorblind |
...
| shade a period | `ax.axvspan(t0, t1, alpha=.12)` |
...
| power-law exponent | polyfit on log10s; slope=b |
