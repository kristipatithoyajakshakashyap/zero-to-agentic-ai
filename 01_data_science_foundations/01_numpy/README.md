...
>  **Common pitfall:** forgetting parentheses in compound masks. Write `(a > 3) & (a < 8)`
> every single time.
...
>  **Common pitfall:** `x = arr.sort()` sets x to None and mutates arr - the method returns
...
>  **Common pitfall:** `inv(A) @ b` works but squares the condition number and computes entries
...
>  **Common pitfall:** creating a fresh unseeded generator inside a loop destroys
...
>  **Common pitfall:** text formats forget dtypes. Anything structural lives happily in
